function [HammingLoss,RankingLoss,OneError,Coverage,Average_Precision,Outputs,Pre_Labels]=CMLKNN_test(train_data,train_target,test_data,test_target,Num,Prior,PriorN,Cond,CondN,maxF,maxFN,m,n,a)
...
%         end
        ...
        for j=1:num_class
            if(j==1)   
                ...
                Prob_in=Prior(j)*(a*Cond(j,temp(1,j)+1) + (1-a)*maxF(j)*Pre_Labels(m(j),i));
                Prob_out=PriorN(j)*(a*CondN(j,temp(1,j)+1) + (1-a)*maxFN(j)*Pre_Labels(n(j),i));
            end
