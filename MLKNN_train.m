function [Prior,PriorN,Cond,CondN]=MLKNN_train(train_data,train_target,Num,Smooth,para_p)
...
%Computing distance between training instances
    ...
    for i=1:num_training-1
        ...
        for j=i+1:num_training            
            ...
            if (para_p==1)
                dist_matrix(i,j)=sum(abs(vector1-vector2));         %Âü¹þ¶Ù¾àÀë
            elseif(para_p==2)
                dist_matrix(i,j)=sqrt(sum((vector1-vector2).^2));   %Å·¼¸ÀïµÃ¾àÀë
            else
                dist_matrix(i,j)=max(abs(vector1-vector2));         %ÇÐ±ÈÑ©·ò¾àÀë 
            end
